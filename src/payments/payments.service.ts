...
export class PaymentsService {
  ...
  async updatePayment(data: UpdatePaymentDTO, id: string) {
    ...
    const updatedBalance = this.calculateBalanceAfterUpdate(
      payment,
      data,
      balance.balance,
    );
  ...
  private updateBalanceOnUpdate(
    payment: Payment,
    updatedData: UpdatePaymentDTO,
    balance: number,
  ...
  private calculateBalanceAfterUpdate(
    payment: Payment,
    updatedData: UpdatePaymentDTO,
    ...
    if (
      ...
      updatedBalance -=
        payment.type === 'EXPENSE' ? amountChange : -amountChange;
    }
  ...
  async deletePayment(id: string) {
    const payment = await this.paymentRepository.findUnique({ id });
    const payments = await this.paymentRepository.findInTime(
      payment.createdAt,
      payment.userId,
    );
    const balance = await this.balanceService.getBalance(payment.userId);

    const updatedBalance = this.calculateBalanceAfterDelete(
      payment,
      payments,
      balance.balance,
    );
  ...
  private calculateBalanceAfterDelete(
    payment: Payment,
    payments: Payment[],
    currentBalance: number,
  ): number {
    const previousPaymentsTotal = payments.reduce(
      (total, p) => total + (p.type === 'EXPENSE' ? p.amount : -p.amount),
      0,
    );
